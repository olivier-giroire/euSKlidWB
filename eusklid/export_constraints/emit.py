...
def add_path_constraints(sk, Sketcher, geom_meta, tol_join):
    ...
    symmetry_y_count = 0
    for left, left_point, right, right_point in mirrored_endpoint_pairs_y_axis(geom_meta, max(float(tol_join), 1e-5)):
        if add_constraint_safe("Symmetric", left["index"], left_point, right["index"], right_point, -2):
            symmetry_y_count += 1
            stats["symmetry_y"] += 1

    symmetry_x_count = 0
    for top, top_point, bottom, bottom_point in mirrored_endpoint_pairs_x_axis(geom_meta, max(float(tol_join), 1e-5)):
    ...
    return {
        ...
        "stats": stats,
        "policy": "minimal-v6.2.1-symmetry-priority",
    }
